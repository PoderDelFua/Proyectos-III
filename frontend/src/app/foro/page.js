                ...
                {hilos.map(hilo => (
                    <div key={hilo._id} className="relative mt-4">
                        <div className="bg-primary-gray p-8 rounded-lg flex items-center">
                            <div className="flex items-center"> 
                                <div className="flex flex-col items-center">
                                    <div className="w-12 h-12 rounded-full overflow-hidden mb-2"> 
                                        <img src="/no-profile.png" alt="Profile" class="w-full h-full object-cover" />
                                    </div>
                                    <p className="text-gray-600 text-sm">@{hilo.creadorId.nickname}</p>
                                </div>
                                <div className="ml-4">
                                    <h1 className="font-bold text-xl text-wrap hover:text-balance">{hilo.titulo}</h1>
                                    <h3 className="text-lg text-wrap mt-4 hover:text-balance">{hilo.descripcion}</h3>
                                </div>
                            </div>
                            <p className="absolute bottom-0 right-0 mb-2 mt-2 mr-10 cursor-pointer">3</p>
                            <button 
                                //falta por implementar el botón de like
                            ...
                            <button 
                                //falta por implementar el botón de respuesta, puede ser también un popup
                                onClick={() => setLike(!like)} 
